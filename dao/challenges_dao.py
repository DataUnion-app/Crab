...
class ChallengesDao(BaseDao):
    ...
    def get_all_challenge(self):
        ...
        res = [{'challenge_id': r['_id'], 'name': r['name'], 'description': r['description'],
                'status': r['status'], 'rules': r['rules'], 'start_date': r['start_date'], 'end_date': r['end_date']}
               for r in result]
        return res

    def update_challenge(self, challenge_id: str, challenge_name: str, status: str, description: str, rules: str):
